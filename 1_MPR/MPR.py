...
class EnhancedMultiViewMedicalImageViewer(QMainWindow):
    ...
    def update_cursor_position(self, view_name, x, y):
        ...
        # Update cursor position while keeping it within bounds
        self.cursor_position[view_name] = QPointF(
            max(0.0, min(1.0, x)),
            max(0.0, min(1.0, y))
        )

        # Calculate voxel coordinates based on normalized cursor position
        if view_name == "axial":
        ...
        # Update cursor positions for other views to maintain synchronization
    ...
    def display_2d_image(self, label, image, view):
        ...
        # Handle view-specific orientations
        ...
        # Get cursor position in normalized coordinates (0-1)
        ...
        visible_width = min(label_size.width(), scaled_pixmap.width())
        visible_height = min(label_size.height(), scaled_pixmap.height())
        
        # Calculate the center point based on cursor position
        center_x = int((scaled_pixmap.width() - visible_width) * cursor_x)
        center_y = int((scaled_pixmap.height() - visible_height) * cursor_y)
        
        # Ensure bounds
        center_x = max(0, min(center_x, scaled_pixmap.width() - visible_width))
        center_y = max(0, min(center_y, scaled_pixmap.height() - visible_height))
        
        # Extract visible portion
        visible_pixmap = scaled_pixmap.copy(center_x, center_y, visible_width, visible_height)
        
        # Set the pixmap
        label.setPixmap(visible_pixmap)
        
        # Calculate crosshair position in label coordinates
        label_width = label.width()
        label_height = label.height()
        
        # Update crosshair position - keep it centered when not moving
        crosshair_x = label_width * cursor_x
        crosshair_y = label_height * cursor_y
        
        label.crosshair_position = QPointF(crosshair_x, crosshair_y)
        label.update()
    ...
    def wheelEvent(self, event):
     ...
     if self.pointer_mode:
        ...
        # Zoom behavior centered on axes
        zoom_speed = 0.1
        old_zoom = self.zoom_factor
        
        # Update zoom factor with bounds
        zoom_delta = 1 + zoom_speed * (delta / 120)
        new_zoom = self.zoom_factor * zoom_delta
        self.zoom_factor = max(1.4, min(5.0, new_zoom))
        
        # Keep cursor positions unchanged since we're centering on axes
        self.update_2d_views()
